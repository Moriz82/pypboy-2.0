            ...
            if self.label:
                if self.label != self.prev_label:
                    ...
                    for section in self.title:
                        text = settings.RobotoB[25].render(section, True, (settings.bright), (0, 0, 0))  # Setup text
        ...
        if not settings.hide_submenu:
            for m in self.menu:
                ...
                while text_width < 54:
                    ...
                    text = settings.RobotoR[20].render("%s%s%s" % (spaces, m, spaces), True, settings.mid, (0, 0, 0))
                    text_width = text.get_size()[0]
                    padding += 1
                # print(m+" : "+str(text.get_size()))
                if m == self.selected:
                    text = settings.RobotoR[20].render("%s%s%s" % (spaces, m, spaces), True, settings.bright, (0, 0, 0))
        ...
        if settings.hide_footer and settings.hide_footer != 3:
            ...
            if self.delta_time >= self.animation_time:
                ...
                if self.sections:
                    ...
                    # Left Text
                    if self.text_left:
                        text = settings.RobotoB[20].render(self.text_left, True, settings.bright, settings.dark)
                    ...
                    if isinstance(self.bar_graph_num, int) and self.bar_graph_centered:
                        text = settings.RobotoB[20].render(self.text_right, True, settings.bright, settings.dark)
                        self.text_width = text.get_rect().width
                        ...
                        text = settings.RobotoB[20].render(self.text_middle, True, settings.bright, settings.dark)
                        self.text_width = text.get_rect().width
    ...
    def redraw(self):
        ...
        for i in range(len(self.menu_array)):
            ...
            if i == self.selected:
                ...
                text = settings.RobotoB[20].render(" %s " % self.menu_array[i][0], True, (0, 0, 0),
                                                   (settings.bright))
                try:
                    number = settings.RobotoB[20].render(" %s " % self.menu_array[i][1], True, (0, 0, 0),
                                                         (settings.bright))
                except:
                    number = ""
                ...
                if stats:
                    ...
                    self.descriptionbox.fill((0, 0, 0))
                    for each in stats:
                        stat_text = settings.RobotoB[20].render(" %s " % each[0], True, (settings.bright),
                                                                (settings.dark))
                        stat_number = settings.RobotoB[20].render(" %s " % each[1], True, (settings.bright),
                                                                  (settings.dark))
                ...
                text = settings.RobotoB[20].render(" %s " % self.menu_array[i][0], True, (settings.bright),
                                                   (0, 0, 0))
                try:
                    number = settings.RobotoB[20].render(" %s " % self.menu_array[i][1], True, (settings.bright),
                                                         (0, 0, 0))
                except:
                    number = None
